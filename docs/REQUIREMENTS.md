...
| AU-12 | Tratamento de desconexão do dispositivo: reabrir até 3×, preservar posição | AUTO | M6 | OK (M6) |
...
| IN-04 | Integração com teclas multimídia | AUTO | M6 | OK (M6 Linux, M8 Windows) — no Windows, verificado em máquina real: play/pause responde e o painel de mídia do sistema mostra título e artista |
| IN-05 | Integração MPRIS no Linux | AUTO | M6 | OK (M6); equivalente no Windows é o SMTC, verificado em máquina real (M8) |
...
| AR-09 | I/O de rede e disco cancelável (`AVIOInterruptCB` + timeout): cancelamento retorna em < 150 ms | AUTO | M1 | OK (M8) — pior caso medido em oito fases: 90 ms; sem o cancelamento, 10 s. O limite era < 100 ms até o M8: mais apertado que o próprio FFmpeg, que consulta o cancelamento a cada 100 ms |
...
| EN-04 | Pacote executável para Linux (AppImage e/ou Flatpak) | MEDIDO | M7 | OK (M7 AppImage, M8 Flatpak) — ambos construídos pelo CI e executados em máquina real |
